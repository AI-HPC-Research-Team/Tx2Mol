...
This implementation includes generation and molecular structural/property evaluation. The independent compound–phenotype compatibility predictor is outside this three-stage release. Exact metric definitions are in [`docs/METHODS.md`](docs/METHODS.md).
...
The historical InfoNCE BOS-embedding fallback is retained and reported by the code. It does not represent whole-molecule contrastive alignment; fixing it would require a separately trained model. See [the implemented protocol](docs/METHODS.md#historical-infonce-limitation).
...
The earlier Linux/CUDA checks and limitations are recorded in [`docs/VALIDATION.md`](docs/VALIDATION.md). Full 2,000/20-epoch training was not repeated during packaging, and installation on a clean host was not independently tested.
...
- [`docs/METHODS.md`](docs/METHODS.md): architecture, losses, tokenization, metrics, and seeds.
- [`docs/PROVENANCE.md`](docs/PROVENANCE.md): historical entry points, integration changes, and provenance limitations.
