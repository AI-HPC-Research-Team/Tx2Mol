...
## Release cleanup (2026-09-25)

The auxiliary LM-loss checkpoint-selection path was removed; post-training always uses the historical generation-based composite score. All 25 CPU regression tests and complete data validation passed after cleanup, and an AST comparison confirmed that the retained generation-based validation block is unchanged. This maintenance check did not rerun GPU training or generation; previous records remain scoped to their recorded source versions.
